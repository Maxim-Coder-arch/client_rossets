...
const ModalOrderProduct = ({ product, onClose }: ModalOrderProductProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/bid-with-goods", {
        method: "POST",
      ...
      <div className="modal-order-product__content">
        ...
        <div className="modal-order-product__product">
          <div className="modal-order-product__product__image">
            <Image src={product.image} alt={product.seriesNumber} width={120} height={120} />
          </div>
          <div className="modal-order-product__product__info">
            <h3>{product.seriesNumber}</h3>
            <p>Серия: {product.rossetSeries}</p>
            <div className="product-details">
              <span>Диаметр: {product.rossetDiameter}см</span>
              <span>Количество хвостов: {product.numberOfTails}</span>
              <span>Длина: {product.tailLength}см</span>
            </div>
